...
parameters{
  matrix<lower=0>[hm_options,hm_attributes] est_trial_option_attribute[hm_trials];//beware that lower bound! Ok here for lengths/areas, not so much in general.
...
model{
  ...
  for(atrial in 1:hm_trials){
	...
	est_trial_option_attribute[atrial,anoption,anattribute]~normal(0,1); //attribute priors. Note scale.
      }
    }
  }
  ...
  for(anobs in 1:hm_calcobs){
    calcobs[calc_trialid[anobs],calc_optionid[anobs]]~normal(est_trial_option_attribute[calc_trialid[anobs],calc_optionid[anobs],1]*est_trial_option_attribute[calc_trialid[anobs],calc_optionid[anobs],2]*0.5,calc_noisesd[anobs]);//est-calcobs consistency is good. Note the .5 specific to triangles A=1/2*w*h
